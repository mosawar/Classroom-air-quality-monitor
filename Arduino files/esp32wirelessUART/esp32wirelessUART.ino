...
//for UART mode 
#define RX_PIN 16 // UART1_RX (connect to MH-Z19 TX)
#define TX_PIN 17 // UART1_TX (connect to MH-Z19 RX)
...
void setup() {
  ...
  // for debugging Serial.begin(115200);
  ...
  if (co2Sensor.getABC()) {
    Serial2.println("ABC Mode is enabled. Disabling...");
...
//temperature = co2Sensor.getTemperature(); // Get temperature reading
  ...
  temperature = dht11.readTemperature(); // Get temperature reading
  ...
  delay(5000); // Wait 1 minute before the next reading
}
